	...
	<div class="section">
		...
		<div class="container">
			...
			<div class="row">
				...
				<jsp:useBean id="mensaje" scope="request" type="java.util.ArrayList<model.Mensaje>"></jsp:useBean>
				<% for(Mensaje v:mensaje){
					if(v.getEmisor().compareTo(usu.getEmail())==0){%>
						<h4 >Tu: <%=v.getMensaje()%></h4>
						<%}
					if(v.getReceptor().compareTo(usu.getEmail())==0){%>
						<h4 ><%=v.getReceptor()%>: <%=v.getMensaje()%></h4>
						<%}%>
